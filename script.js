...
function crearGraficos(){
  ...
  charts.topReincidentes=new Chart(document.getElementById('graficoTopReincidentes').getContext('2d'),{
    type:'bar', data:{labels:[],datasets:[{label:'Averías',data:[],backgroundColor:[],borderWidth:0,borderRadius:3}]},
    options:{
      responsive:true, maintainAspectRatio:false, indexAxis:'y',
...
function inicializarTabla(){
  ...
  tablaDT=$('#tablaAverias').DataTable({
    ...
    orderCellsTop:true, colReorder:true, scrollX:true, pageLength:25, lengthChange:true,
  ...
  // ═══════ INPUT DE PÁGINA MANUAL - SE CREA DESPUÉS DE CADA REDIBUJADO ═══════
  function crearInputPaginaManual(){
    ...
    wrap.style.cssText='display:inline-flex;align-items:center;gap:6px;margin-left:12px;color:var(--text-muted);font-size:12px;font-family:Inter,sans-serif;vertical-align:middle;';
    ...
    wrap.innerHTML=`
      <span>Ir a:</span>
      <input type="number" id="inputPagina" min="1" max="${total}" value="${pagActual}" 
        style="width:55px;padding:5px 8px;background:var(--input-bg);color:var(--input-text);border:1px solid var(--input-border);border-radius:6px;font-size:12px;font-family:Inter,sans-serif;text-align:center;"
        title="Escribe el número de página y pulsa Enter">
      <button id="btnIrPagina" 
        style="padding:5px 10px;background:var(--accent);color:var(--bg);border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;"
        title="Ir a la página">Ir</button>
      <span class="total-pag" style="color:var(--text-muted);font-size:11px;">de <strong id="totalPaginas">${total}</strong></span>
    `;
  ...
  // Crear input después de cada redibujado de la tabla
  $('#tablaAverias').on('draw.dt',function(){
    setTimeout(crearInputPaginaManual,100);
  });
  
  // Crear input inicialmente
...
function actualizarGraficos(){
  ...
  if(charts.recurrencia){
    ...
    let unicos=0, reincidentes=0;
    Object.values(vhCount).forEach(n=>{if(n===1)unicos++;else if(n>=2)reincidentes++;});
  ...
  if(charts.topReincidentes){
    ...
    charts.topReincidentes.data.datasets[0].backgroundColor=bg;
    charts.topReincidentes.options.plugins.legend.labels.color=getLegendColor();
...
function cerrarMenus(){
  ...
  if(m)m.remove();
  const m2=document.getElementById('menuColumnas');
  if(m2)m2.remove();
}
...
function iniciarResizeAltura(){
  ...
  document.addEventListener('mousemove',e=>{
    ...
    if(dir==='s'||dir==='se')panel.style.height=Math.max(200,startH+deltaY)+'px';
    if(dir==='e'||dir==='se')panel.style.flex=`0 0 ${Math.max(200,startW+delta)}px`;
  });
